...
public:
    ...
    vector<string> findAllConcatenatedWordsInADict(vector<string>& words) {
        ...
        for(string str:words){
            TrieNode *loop = root;
            for(char c:str){
                if(loop->chs[c-'a'] == NULL) loop->chs[c-'a'] = new TrieNode();
                loop = loop->chs[c-'a'];
            }
            loop->used = true;
        }
...
int main(int argc,char *argv[]){
    ...
    Solution s;
    vector<string> ans = s.findAllConcatenatedWordsInADict(test);
    for(string s:ans){
        cout << s << endl;
    }
    return 0;
